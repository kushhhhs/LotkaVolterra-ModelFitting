...
def remove_random_points(t_obs, data, num_points):
    ...
    for _ in range(num_points):    
        ...
        print(f"Removed point at index {index_removed}: t = {t_obs[index_removed]:.2f}, x = {data[index_removed]:.3f}")
    
    return t_reduced, data_reduced, removed_indices

def interpolate_points(t_reduced, t_obs, reduced_data, index_removed):
    ...
    print(f"Interpolated value at t = {t_obs[index_removed]:.2f}: {x_interpolated:.3f}")
